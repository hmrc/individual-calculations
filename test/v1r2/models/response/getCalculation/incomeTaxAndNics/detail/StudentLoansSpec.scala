...
import v1r2.fixtures.getCalculation.incomeTaxAndNics.detail.StudentLoanFixtures._
  ...
  "StudentLoans" should {
    "read from json correctly" when {
      "provided with valid json" in {
        studentLoansJson.as[StudentLoans] shouldBe studentLoansModel
      }
    }

    "write to json correctly" when {
      "a valid model is provided" in {
        Json.toJson(studentLoansModel) shouldBe studentLoansJson
      }
    }
