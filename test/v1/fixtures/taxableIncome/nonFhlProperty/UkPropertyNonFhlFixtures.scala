...
import v1.models.response.getCalculation.taxableIncome.nonFhlProperty.UkPropertyNonFhl
...
object UkPropertyNonFhlFixtures {
  ...
  val ukPropertyNonFhlDesJson: JsValue = Json.parse(
    s"""
      |{
      | "calculation" : {
      |   "businessProfitAndLoss" : [
      |     ${businessProfitAndLossJson("02").toString()},
      |     ${businessProfitAndLossJson("04").toString()}
      |   ]
      | }
      |}
    """.stripMargin).asInstanceOf[JsObject].deepMerge(LossClaimsDetailFixtures.lossClaimsDetailDesJson.asInstanceOf[JsObject])

  val ukPropertyNonFhlDesJsonWithoutValidTypes: JsValue = Json.parse(
    s"""
       |{
       | "calculation" : {
       |   "businessProfitAndLoss" : [
       ...
       | }
       |}
    """.stripMargin).asInstanceOf[JsObject].deepMerge(LossClaimsDetailFixtures.lossClaimsDetailDesJsonWithoutValidTypes.asInstanceOf[JsObject])

  val ukPropertyNonFhlDesJsonWithoutOptionals: JsValue = Json.parse(
    s"""
       |{
       | "calculation" : {
       |   "businessProfitAndLoss" : [
       ...
       | }
       |}
    """.stripMargin).asInstanceOf[JsObject].deepMerge(LossClaimsDetailFixtures.lossClaimsDetailDesJsonWithoutOptionals.asInstanceOf[JsObject])
