...
import v1.models.response.common.taxableIncome.detail.payPensionsProfit.{ BusinessProfitAndLoss, SelfEmployments, UkPropertyFhl, UkPropertyNonFhl }
import v1.models.response.common.taxableIncome.detail.{ Dividends, PayPensionsProfit, Savings, SavingsAndGains }

class DetailSpec extends UnitSpec {
  ...
  val detailResponse                                       = Detail(Some(payPensionsProfitResponse), Some(savingsAndGainsResponse), Some(dividendsResponse))
  val emptyDetailResponse                                  = Detail(None, None, None)

  "Detail" when {
    "read from valid Json" should {
      "return a JsSuccess" in {
        desJson.validate[Detail] shouldBe a[JsSuccess[_]]
      }
      "with the expected Detail object" in {
        desJson.as[Detail] shouldBe detailResponse
      }
    }
    "read from empty Json" should {
      "return a JsSuccess" in {
        emptyJson.validate[Detail] shouldBe a[JsSuccess[_]]
      }
      "with the expected Detail object" in {
        emptyJson.as[Detail] shouldBe emptyDetailResponse
      }
    }
