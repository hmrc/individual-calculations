...
class GainsOnLifePoliciesSpec extends UnitSpec {
  ...
  "GainsOnLifePolicies" when {
    ...
    "read from invalid JSON" should {
      "return a JsError" in {
        ...
        invalidDesJson.validate[GainsOnLifePolicies] shouldBe a[JsError]
      }
    }

    "written to JSON" should {
      "produce the expected JsObject" in {
        val mtdJson: JsValue = (TaxableIncomeJsonFixture.mtdJson \ "detail" \ "gainsOnLifePolicies").get
        Json.toJson(TaxableIncomeModelsFixture.gainsOnLifePoliciesModel) shouldBe mtdJson
      }
    }
  }
}
